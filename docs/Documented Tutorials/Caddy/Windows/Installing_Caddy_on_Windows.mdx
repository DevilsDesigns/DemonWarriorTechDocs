...
- **Static IP address**: Your host machine should have a static IP configured for reliable server operation and port forwarding. See [How to Set a Static IP on Windows](../../Troubleshooting#how-to-set-a-static-ip-on-windows) for setup instructions.
- **Port access**: Ability to bind to ports 80 and 443
- **Firewall configuration**: Permissions to modify Windows Firewall rules
...
For production use, install Caddy as a Windows service using NSSM (Non-Sucking Service Manager):

**See: [Setting Up NSSM for Caddy on Windows](./How_to_Setup_NSSM)** for complete service installation instructions including:
- Automatic startup on boot
- Service monitoring and restart on failure
...
:::warning
You must create a **[User API Token](https://dash.cloudflare.com/profile/api-tokens)**, not an Account API Token. Cloudflare Account API Tokens will not work for this configuration.
:::
...
:::danger
If Using a VPN, following the steps below can break your DNS configuration
and make your services unreachable, continue with caution.
:::

#### Download CloudFlare DDNS

1. **Download Cloudflare DDNS**: [CloudFlare DDNS](https://github.com/wyattjoh/cloudflare-ddns/releases)
   - Choose the package for your system: Most likely `cloudflare-ddns_v1.1.1_windows_amd64.zip`

![Assets](/img/Cloudflare/DDNS_Assets.png)

2. **Extract to directory**: `C:\Tools\CloudflareDDNS`
   - The Filepath after being unzipped should be as follows.

<Trees title="C:\Tools\CloudflareDDNS">
  <Folder label="C:" expanded>
    <Folder label="Tools" expanded>
      <Folder label="CloudFlareDDNS" expanded>
        <File label="cloudflare-ddns.exe" />
      </Folder>
    </Folder>
  </Folder>
</Trees>
...
1. **Open a Terminal Window**:
   - Right click any blank space in the **CloudFlareDDNS** Directory.
...
3. **Open `cloudflare-ddns-automated.bat`** in a text editor and change these lines:
   ...
   ```powershell title="cloudflare-ddns-automated.bat" showLineNumbers
...
![Create Task](/img/Cloudflare/DDNS_CreateTask.png)
...
![Task General](/img/Cloudflare/DDNS_TaskGeneral.png)
...
![Task Trigger 1](/img/Cloudflare/DDNS_TaskTrigger1.png)
![Task Trigger 2](/img/Cloudflare/DDNS_TaskTrigger2.png)
...
- **Program/script:** `C:\Tools\CloudFlareDDNS\cloudflare-ddns-automated.bat`
- **Start in:** `C:\Tools\CloudFlareDDNS`

![Task Actions](/img/Cloudflare/DDNS_TaskActions.png)
...
![Task Conditions](/img/Cloudflare/DDNS_TaskConditions.png)
...
![Task Settings](/img/Cloudflare/DDNS_TaskSettings.png)
...
![Task Run](/img/Cloudflare/DDNS_TaskRun.png)

**Your DDNS will now update automatically every 5 minutes!** 🚀
