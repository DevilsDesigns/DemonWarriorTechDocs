...
|---|---|
| `sudo ufw allow ssh comment 'SSH'` | Allows SSH access — **run this first** to avoid being locked out |
...
| `sudo ufw allow 80/tcp comment 'HTTP'` | Opens port 80 for HTTP traffic |
| `sudo ufw allow 443/tcp comment 'HTTPS'` | Opens port 443 for HTTPS traffic |
| `sudo ufw enable` | Activates the firewall and applies all rules |
